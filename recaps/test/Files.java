...
class Folder implements Item{
    ...
    public void addFileOrFolder(Item itemToAdd){
        if(itemToAdd!=null)
        {
            Item[] newEntries=new Item[nrOfEntries+1];
            for(int i=0;i<nrOfEntries;i++){
                newEntries[i]=entries[i];
         }
         entries=newEntries;
         entries[nrOfEntries++]=itemToAdd;
        }
    }
...
class MainFile{
    public static void main(String [] args){
        ...
        folder1.addFileOrFolder(f1);
        folder1.addFileOrFolder(f2);

        folder2.addFileOrFolder(folder1);
        folder2.addFileOrFolder(f3);
